/*************************************************************************************
							This file is the GD32E103 GD seiral
						chip bootloader source C file, edited by Xieyuyin

Sketch:	Power on boot communicte parameter: 9600 8 N 1, Over 1s System read customer
		parameters area value, according to customized parameters work
*************************************************************************************/
//Include files
#include "Usart.h"
#include "Dmaconfig.h"
...
#define	MARSK_GD32E103_FWDRST 		0x20000000
#define	MARSK_GD32E103_CLRALLRSTFLG	0x01000000
...
static void GD32E103BOOT_ResourceInit( void );
static void GD32E103_TestLedInit( void );
static bool CB_DisplayLedBlink( void* pdat );

static void Boot_ParseReceiveModbusData( u8* psrc, u32 rcvlen, u8 msgtype );

static bool GD32E103BOOT_1sOverTimePorcess( void* pdat );
static bool GD32E103BOOT_ClearAppFlashArea( void );
static bool GD32E103BOOT_UpGradePayloadProcess( u8* psrc, u16 datlen );
static void GD32E103BOOT_UpgradePackagePayloadDecoding( u8* psrc, u16 srclen );
static void GD32E103BOOT_EraseUpgradeFlag( void );
...
//#ifdef USE_FDWGD
//		GD32E103_FEEDWATCHDOG();
//#endif
		
		GD32E103_Systick_TimerProcess( );
	}
}
...
//	GD32E103_TestLedInit( );
//	GD32E103_TimerSet( eTmr_LedBlinkTimer, CB_DisplayLedBlink, NULL, 1000 );
//	
//#ifdef USE_FDWGD
//	GD32E103_FDWDGInit( );
//#endif
	...
	GD32E103_TimerSet( eTmr_BootLoader, GD32E103BOOT_1sOverTimePorcess, NULL, 1000  );
...
static void Boot_ParseReceiveModbusData( u8* psrc, u32 rcvlen, u8 msgtype )
{
    ...
    {
        flg_boot1s_ot = 0;
        GD32E103_Systick_TimerKill( eTmr_BootLoader );
    }
    ...
    case CMD_UPGRADE_START:
        *(ptmp + 3) = 0x00;
        GD32E10X_Modbus_UpgradeResponseProcess( ptmp, 1 );
        break;
    ...
    case CMD_UPGRADE_CLRCODE:   //Clear flash code area
        ...
        GD32E103BOOT_ClearAppFlashArea();
        *(ptmp + 3) = 0x00;
        GD32E10X_Modbus_UpgradeResponseProcess( ptmp, 1 );
        break;

    case CMD_UPGRADE_PAYLOAD:
        if (s_upp.start && GD32E103BOOT_UpGradePayloadProcess(psrc, datlen))
        {
            *(ptmp + 3) = 0x00;
            GD32E10X_Modbus_UpgradeResponseProcess( ptmp, 1 );
        }
    ...
    case CMD_UPGRADE_RD_PARA:
    {
        ...
        GD32E10X_Modbus_UpgradeResponseProcess( ptmp, UPLOADDATA_LEN );
        break;
    }
...
static bool GD32E103BOOT_1sOverTimePorcess( void* pdat )
{
    ...
    {	
        ...
        if( usrsetp[3] < 0 || usrsetp[3] >= 0xFF)
        {		
			commpara.SlaveAddr = 0x41;
        }
        else
        {
			commpara.SlaveAddr = usrsetp[3];
        }
...
static bool GD32E103BOOT_UpGradePayloadProcess( u8* psrc, u16 datlen )
{
    ...
    GD32E103BOOT_UpgradePackagePayloadDecoding( psrc, datlen );
    //Write data to flash
...
Retn:   
*******************************************************************************/
#define UPGRADE_FLAG_OFFSET 0
static void GD32E103BOOT_EraseUpgradeFlag( void )    //Erase upgrade flag
{
    memset( gtmpbuf, 0, TMPBUF_SIZE );
    GDFLASHOP_ReadDataFromChipFlash( gtmpbuf, MAGNET_PARAMETER_ADDR, DWTMPBUF_SIZE, DWTMPBUF_SIZE );
    if (gtmpbuf[UPGRADE_FLAG_OFFSET] == UPGRADE_FALG)
    {
        gtmpbuf[UPGRADE_FLAG_OFFSET] = 0x00;
        GDFLASHOP_EraseSpecifiedFlashPages( MAGNET_PARAMETER_ADDR, 1 );
        GDFLASHOP_WriteData2SpecifiedFlash( MAGNET_PARAMETER_ADDR, (u32*)gtmpbuf, DWTMPBUF_SIZE);
    }
}
